        ...
        <div class="box p-1">
                ...
                <div class="table-container mt-2 mx-2" >
                    <table class="table  is-striped is-bordered" >
                        <thead>
                            <th class="has-text-info">Nombres, cliente</th>
                            <th class="has-text-info">Apellido paterno, cliente</th>
                            <th class="has-text-info">Apellido materno, cliente</th>
                        </thead>
                        <tbody>
                            <tr>
                ...
                <div class="is-flex is-flex-direction-column">
                    <v-text-field v-model="item_modificacion_disenio.fecha" label="Fecha" class="m-1"
                        color="indigo-lighten-1" :error-messages="getViewMessageErrorsField('fecha')" type="date" />

                    <v-textarea v-model="item_modificacion_disenio.observacion" label="Observacion" class="m-1"
                        color="indigo-lighten-1" :error-messages="getViewMessageErrorsField('observacion')" />
                </div>

        

            <div class="is-flex is-flex-wrap-wrap   is-justify-content-center is-align-items-center m-1">
                ...
                <v-btn @click="save()" color="indigo-lighten-1" class="as-hover__box-shadow m-1 outlined">
                    <span class="mdi mdi-content-save-all is-size-6"></span>&nbsp;Guardar modificacion
                </v-btn>
...
const item_modificacion_disenio = ref(new ModificacionDisenio().getFill());
...
const confirmDeleteData = async () => {
    const revision = new ModificacionDisenio(props.item_disenio_prop, item_modificacion_disenio.value);
...
const openDeleteData = (item) => {
    ...
    item_modificacion_disenio.value = Object.assign({}, item);
...
const newForm = () => {
    ...
    indice_data.value = -1;
    item_modificacion_disenio.value = new ModificacionDisenio().getFillShe();
};
...
const initData = () => {
    ...
    setTimeout(async () => {
        const revision = new ModificacionDisenio(props.item_disenio_prop);
